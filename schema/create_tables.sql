...
CREATE TABLE ANGAJATI (
    ...
    CONSTRAINT angajat_depozit_fk FOREIGN KEY (id_depozit) REFERENCES DEPOZIT(id_depozit)
);
...
CREATE TABLE PRODUSE (
    ...
    CONSTRAINT produs_producator_fk FOREIGN KEY (id_producator) REFERENCES PRODUCATOR(id_producator),
    CONSTRAINT produs_categorie_fk FOREIGN KEY (id_categorie) REFERENCES CATEGORIE(id_categorie)
);
...
CREATE TABLE PRODUS_COMANDA (
    ...
    CONSTRAINT produs_comanda_pk PRIMARY KEY (id_produs, id_comanda),
    CONSTRAINT produs_comanda_produs_fk FOREIGN KEY (id_produs) REFERENCES PRODUS(id_produs),
    CONSTRAINT produs_comanda_comanda_fk FOREIGN KEY (id_comanda) REFERENCES COMANDA(id_comanda)
);
...
CREATE TABLE COMENZI (
    ...
    CONSTRAINT comanda_factura_fk FOREIGN KEY (id_factura) REFERENCES FACTURA(id_factura)
);
...
CREATE TABLE RUTE (
    ...
    CONSTRAINT ruta_depozit_fk FOREIGN KEY (id_depozit) REFERENCES DEPOZIT(id_depozit),
    CONSTRAINT ruta_comanda_fk FOREIGN KEY (id_comanda) REFERENCES COMANDA(id_comanda),
    CONSTRAINT ruta_sofer_fk FOREIGN KEY (id_sofer) REFERENCES SOFER(id_angajat)
);
...
CREATE TABLE MANIPULANTI (
    ...
    CONSTRAINT manipulant_angajat_fk FOREIGN KEY (id_angajat) REFERENCES ANGAJAT(id_angajat)
);
...
CREATE TABLE SOFERI (
    ...
    CONSTRAINT sofer_angajat_fk FOREIGN KEY (id_angajat) REFERENCES ANGAJAT(id_angajat)
);
